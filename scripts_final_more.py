"""Small refinements: LGBM lr 0.01 k20 bag, CatBoost lr 0.05 k20 seed."""
...
S_LGB = {"max_depth": 5, "num_leaves": 32, "min_child_samples": 10, "feature_fraction": 0.3, "bagging_fraction": 0.8,
         "lambda_l1": 0.071, "lambda_l2": 2.0, "max_bin": 1024, "learning_rate": 0.01}
S_CB = {"depth": 5, "rsm": 0.3, "l2_leaf_reg": 2.0, "learning_rate": 0.05, "border_count": 1024}
def bag(out, names, k):
...
stage = sys.argv[1]
if stage == "cb05":
    run("cbF_k20_lr05_s0", full, te_specs=TE, model="cb", static_version="v2", n_folds=20, params=S_CB, ref="cbF_k20_s0")
elif stage == "lgb01":
    names = []
    for s in (0, 1, 2):
        run(f"lgbG_k20_s{s}", full, te_specs=TE, static_version="v2", n_folds=20, seed_te=s, params={**S_LGB, "seed": s}, ref=f"lgbF_k20_s{s}")
        names.append(f"lgbG_k20_s{s}")
    bag("lgbG_k20_bag3", names, 20)
