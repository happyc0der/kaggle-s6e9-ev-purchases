...
if stage in ("nn", "all"):
    run("nn_full", full, te_specs=te_multi, model="nn", static_version="v2", ref="lgb_full_temulti")
if stage in ("cb", "all"):
    ...
    run("cb_full_cat", full, te_specs=te1, model="cb", static_version="v2", cat_cols=[INC, COM],
        params={"learning_rate": 0.06, "depth": 8}, ref="lgb_full_temulti")
if stage in ("xgb", "all"):
    run("xgb_full", full, te_specs=te_multi, model="xgb", static_version="v2", params={"max_bin": 1024}, ref="lgb_full_temulti")
